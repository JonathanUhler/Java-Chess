...
public class AI {
    ...
    private Board ghostBoard = new Board();
    private int movesFound = 0;

    // Rewards and penalties to incentivize a certain style of play
    private final double aiCaptureIncentive = 0.1; // Small reward for a move that results in a capture for the AI
    private final double positionalPlayIncentive = 0.005; // Multiplier for the values in the tables below
    private final double[] pawnPositionIncentives = new double[] {0, 0, 0, 0, 0, 0, 0, 0, 50, 50, 50, 50, 50, 50, 50, 50, 10, 10, 20, 30, 30, 20, 10, 10, 5, 5, 10, 25, 25, 10, 5, 5, 0, 0, 0, 20, 20, 0, 0, 0, 5, -5, -10, 0, 0, -10, 5, 5, 5, 10, 10, -20, -20, 10, 10, 5, 0, 0, 0, 0, 0, 0, 0, 0};
    private final double[] knightPositionIncentives = new double[] {-50, -40, -30, -30, -30, -30, -40, -50, -40, -20, 0, 0, 0, 0, -20, -40, -30, 0, 10, 15, 15, 10, 0, -30, -30, 5, 15, 20, 20, 15, 5, -30, -30, 0, 15, 20, 20, 15, 0, -30, -30, 5, 10, 15, 15, 10, 5, -30, -40, -20, 0, 5, 5, 0, -20, -40, -50, -40, -30, -30, -30, -30, -40, -50};
    private final double[] bishopPositionIncentives = new double[] {-20, -10, -10, -10, -10, -10, -10, -20, -10, 0, 0, 0, 0, 0, 0, -10, -10, 0, 5, 10, 10, 5, 0, -10, -10, 5, 5, 10, 10, 5, 5, -10, -10, 0, 10, 10, 10, 10, 0, -10, -10, 10, 10, 10, 10, 10, 10, -10, -10, 5, 0, 0, 0, 0, 5, -10, -20, -10, -10, -10, -10, -10, -10, -20};
    private final double[] rookPositionIncentives = new double[] {0, 0, 0, 0, 0, 0, 0, 0, 5, 10, 10, 10, 10, 10, 10, 5, -5, 0, 0, 0, 0, 0, 0, -5, -5, 0, 0, 0, 0, 0, 0, -5, -5, 0, 0, 0, 0, 0, 0, -5, -5, 0, 0, 0, 0, 0, 0, -5, -5, 0, 0, 0, 0, 0, 0, -5, 0, 0, 0, 5, 5, 0, 0, 0};
    private final double[] queenPositionIncentives = new double[] {-20, -10, -10, -5, -5, -10, -10, -20, -10, 0, 0, 0, 0, 0, 0, -10, -10, 0, 5, 5, 5, 5, 0, -10, -5, 0, 5, 5, 5, 5, 0, -5, 0, 0, 5, 5, 5, 5, 0, -5, -10, 5, 5, 5, 5, 5, 0, -10, -10, 0, 5, 0, 0, 0, 0, -10, -20, -10, -10, -5, -5, -10, -10, -20};
    private final double[] kingPositionIncentives = new double[] {-30, -40, -40, -50, -50, -40, -40, -30, -30, -40, -40, -50, -50, -40, -40, -30, -30, -40, -40, -50, -50, -40, -40, -30, -30, -40, -40, -50, -50, -40, -40, -30, -20, -30, -30, -40, -40, -30, -30, -20, -10, -20, -20, -20, -20, -20, -20, -10, 20, 20, 0, 0, 0, 0, 20, 20, 20, 30, 10, 0, 0, 10, 30, 20};
    ...
    //
    private double getMoveIncentive(int piece, Move move) {
        double incentive;
    ...
    //
    private List<Move> orderMoves(List<Move> movesToOrder, Board board) {
        HashMap<Move, Integer> scoredMoves = new HashMap<>();

        for (Move move : movesToOrder) {
            int moveScore;
            int movePiece = Piece.pieceType(board.getTile()[move.startTile()]);
            int capturedPiece = Piece.pieceType(board.getTile()[move.endTile()]);
    ...
    //
    private double search(int currentLevel, int maxDepth, Move move, double runningTotal) {
        ...
        if (Move.isCapture(move, ghostBoard)) {
            incrementalVal += Piece.getValue(Piece.pieceType(ghostBoard.getTile()[move.endTile()]));
        ...
        if (currentLevel % 2 == 1) {
            incrementalVal += getMoveIncentive(Piece.pieceType(ghostBoard.getTile()[move.startTile()]), move);
        }

        // Make move
        ghostBoard.makeMove(move, true);
        MoveUtility lookForCheck = new MoveUtility();
        ghostBoard.getTilesOpponentControls().clear();
        ghostBoard.getTilesOpponentControls().addAll(MoveUtility.returnEndingTiles(lookForCheck.generateMoves(ghostBoard)));

        // Check for draw by threefold repetition
        if (ghostBoard.getThreeFoldRepetition().containsValue(3)) {
            return 0;
        }
    ...
    public void makeMove() {
        ...
        List<Move> aiMoves = aiMoveUtil.allLegalMoves(Chess.getBoard());
        ...
        if (bestMove != null) {
            ...
            Chess.getBoard().makeMove(bestMove, false);
            Chess.getGraphics().postMoveUpdates();
            Chess.getGraphics().drawBoard(new ArrayList<>(Arrays.asList(63 - bestMove.startTile(), 63 - bestMove.endTile())));
        }
    }
