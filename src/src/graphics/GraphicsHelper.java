...
import util.LegalMoveUtility;
import util.MoveUtility;
import util.PromotionUtility;
import ai.AI;
import javax.swing.*;
import java.awt.*;
...
public class GraphicsHelper {
    ...
    ArrayList<Integer> legalMoveTiles; // List of legal ending moves
    List<Move> legalMoves; // List of fully legal moves
    ...
    public void drawPosition() {
        ...
        for (PieceTracker pieceTracker : Chess.getBoard().getAllPieceTrackers()) {
            for (int i = 0; i < pieceTracker.getPieceCount(); i++) {
                String pieceBinaryIdentifier = Integer.toBinaryString(pieceTracker.getPieceColor() | pieceTracker.getPieceType()); // Define the binary string for the piece (that is the color | the type)
                JLabel piece = new JLabel(new ImageIcon(Chess.getBoard().getChessProjectPath() + "/reference/pieces/" + pieceBinaryIdentifier + ".png")); // Create a new label with the correct image
                piece.setBounds(pieceStartingX * ((pieceTracker.getTilesWithPieces()[i] % 8) + 1), pieceStartingY * (int) ((Math.floor(pieceTracker.getTilesWithPieces()[i] / 8.0)) + 1), pieceW, pieceH); // Set the size and position of the piece
                ...
                piece.addMouseListener(new MouseAdapter() {
                    @Override public void mousePressed(MouseEvent e) {
                        legalMoveTiles = new ArrayList<>();
                        legalMoves = new ArrayList<>();
                        GraphicsHelper.this.mousePressed(e, piece);
                    }
                });
                ...
                piece.addMouseMotionListener(new MouseMotionAdapter() {
                    @Override public void mouseDragged(MouseEvent e) {
                        GraphicsHelper.this.mouseDragged(e, piece);
                    }
                });
                ...
                piece.addMouseListener(new MouseAdapter() {
                    @Override public void mouseReleased(MouseEvent e) {
                        GraphicsHelper.this.mouseReleased(piece, finalI, pieceTracker);
                    }
                });
    ...
    public void drawBoard(ArrayList<Integer> highlightTiles) {
        ...
        // Brown theme
        Color lightColor = new Color(236, 217, 185);
        Color darkColor = new Color(175, 137, 104);
        ...
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                ...
                if ((row + col) % 2 == 1) {
                    c = (highlightTiles.contains(((row * 8) + col)) && Chess.getBoard().getShowLegalMoves()) ? new Color(161, 86, 86) : darkColor;
                }
                // Otherwise, set it to the other color
                else {
                    c = (highlightTiles.contains(((row * 8) + col)) && Chess.getBoard().getShowLegalMoves()) ? new Color(238, 156, 156) : lightColor;
                }
                ...
                JLabel newTile = new JLabel((DEBUG_MODE) ? String.valueOf(row * 8 + col) : ""); // Create a new tile object to add to the layered pane
                newTile.setBorder(BorderFactory.createLineBorder(c, (DEBUG_MODE) ? 5 : (w / 2))); // Set the color of the tile
    ...
    public void createApplication() {
        ...
        appWindow.setBounds(0, 0, w, h); // Set the size of the frame
    ...
    //
    // Returns the position of the chess window
    //
    // Arguments--
    //
    // None
    ...
    // end: public Point getWindowPosition


    // ====================================================================================================
    // void mousePressed
    //
    // Acts on a mouse press event
    //
    // Arguments--
    //
    // e:       the mouse event
    //
    // piece:   the piece that was pressed
    //
    // Returns--
    //
    // None
    //
    void mousePressed(MouseEvent e, JLabel piece) {
        ...
        legalMoves.addAll(legalMoveUtil.allLegalMoves(Chess.getBoard()));

        // Get the tiles for the piece selected by the player
        for (Move legalMove : legalMoves) {
            int legalMoveValue = legalMove.getMoveValue();
    ...
    // void mouseDragged
    //
    // Acts on a mouse drag event
    //
    // Arguments--
    //
    // e:       the mouse event
    //
    // piece:   the piece that was dragged
    //
    // Returns--
    //
    // None
    //
    void mouseDragged(MouseEvent e, JLabel piece) {
    ...
    // void mouseReleased
    //
    // Acts on a mouse release event
    //
    // Arguments--
    //
    // piece:   the piece that was released
    //
    // index:   the index of the piece
    //
    // tracker: the PieceTracker the piece belongs to
    //
    // Returns--
    //
    // None
    //
    void mouseReleased(JLabel piece, int index, PieceTracker tracker) {
        int moveFlag = Move.Flag.none;
        int startTile = tracker.getTilesWithPieces()[index];
        int endTile = (int) (Math.round((piece.getLocation().y / 72.0) - 1) * 8 + Math.round((piece.getLocation().x / 72.0) - 1));

        // Figure out move flag
        // En passant and promotion
        if (Piece.pieceType(Chess.getBoard().getTile()[startTile]) == Piece.Pawn) {
            // Promotion
            ...
            // Pawn pushed two
            if ((endTile + 16) == tracker.getTilesWithPieces()[index]) {
        ...
        // Castling
        else if (Piece.pieceType(Chess.getBoard().getTile()[startTile]) == Piece.King) {
            // White king castle
            if (Chess.getBoard().getColorToMove() == 0) {
        ...
        List<Move> pseudoLegalMoves = checkMyMoves.generateMoves(Chess.getBoard());
        ...
        if (pseudoLegalMoveValues.contains(move.getMoveValue())) {
            Board ghostBoard = new Board(); // Deep copy the board
            try { ghostBoard = (Board) Chess.getBoard().clone(); }
            ...
            if (opponentAttackedTiles.contains(ghostBoard.getKings()[Chess.getBoard().getColorToMove()].getTilesWithPieces()[0])) {
                Chess.getGraphics().drawPosition(); // Redraw the board
                Chess.getGraphics().drawBoard(null); // Redraw the board
            }
            // The move was completely legal
            else {
                // Update internal data-structure
                Chess.getBoard().makeMove(move, false);
                postMoveUpdates();

                // If the AI is enabled, make its move
                AI ai = new AI();
                if (Chess.getBoard().getEnableAI()) { ai.makeMove(); }
            }
        }
    ...
    public void postMoveUpdates() {
        ...
        Chess.getBoard().getTilesOpponentControls().clear();
        for (int i : MoveUtility.returnEndingTiles(lookForCheck.generateMoves(Chess.getBoard()))) { Chess.getBoard().getTilesOpponentControls().add(63 - i); }

        // Update visual representation of the board
        Chess.getBoard().changePlayer(); // Change information about whose turn it is
        Settings.drawSettings(); // Update the fen string in the text field
        Chess.getBoard().loadPosition(FenUtility.changePlayerPerspective(FenUtility.buildFenFromPosition())); // Load the new position
        drawPosition(); // Draw the position
        drawBoard(null); // Draw the board
        Chess.getBoard().checkState(); // Check on the state of the game
    }
