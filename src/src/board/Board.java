...
public class Board implements Cloneable {
    ...
    private String chessProjectPath;
    {try {chessProjectPath = new File(new File(new File(Board.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getPath()).getAbsolutePath()).getParent();} catch (URISyntaxException e) {e.printStackTrace();}} // Get the path for .../Chess/
    ...
    private int[] tile = new int[64]; // Every tile on the board

    private PieceTracker[] pawns; // 1 PieceTracker for all white pawns, 1 PieceTracker for all black pawns
    private PieceTracker[] knights; // 1 PieceTracker for all white knights, 1 PieceTracker for all black knights
    private PieceTracker[] bishops; // 1 PieceTracker for all white bishops, 1 PieceTracker for all black bishops
    private PieceTracker[] rooks; // 1 PieceTracker for all white rooks, 1 PieceTracker for all black rooks
    private PieceTracker[] queens; // 1 PieceTracker for all white queens, 1 PieceTracker for all black queens
    private PieceTracker[] kings; // 2 integers. 1 for the tile of the white king, 1 for the tile of the black king
    private PieceTracker[] allPieceTrackers; // Every piece for both white and black

    private int fullmoves; // Number of fullmoves played this game
    private int fiftyMoveRule; // Number of fullmoves since the last pawn movement or piece capture
    private String enPassantTile = "-"; // Tile for en passant
    private int enPassantCol = -1; // Column for en passant
    private boolean castleK; // White castle kingside
    private boolean castleQ; // White castle queenside
    private boolean castlek; // Black castle kingside
    private boolean castleq; // Black castle queenside

    private String currentFenPosition; // Current arrangement of pieces on the board in fen notation
    private HashMap<String, Integer> threeFoldRepetition = new HashMap<>(); // List of positions and how many times they have appeared in the game

    private List<Integer> tilesOpponentControls = new ArrayList<>(); // List of the tiles the opponent attacks

    private boolean whitesMove; // Is white to move?
    private int colorToMove; // Which color is to move
    private int opponentColor; // What is the opposing color
    private int friendlyColor; // What is the safe color
    private int pawnDir = -1; // What direction do pawns move (up or down)?

    private boolean showLegalMoves = true; // Should legal moves be highlighted?
    private boolean enableAI = false; // Should the AI player be enabled?


    // ====================================================================================================
    // GET methods
    public String getChessProjectPath() {
        return chessProjectPath;
    }
    ...
    public PieceTracker[] getKings() {
        return kings;
    }

    public PieceTracker[] getAllPieceTrackers() {
        return allPieceTrackers;
    }
    ...
    public String getEnPassantTile() {
    ...
    public boolean getCastleK() {
        return castleK;
    }

    public boolean getCastleQ() {
        return castleQ;
    }
    ...
    public HashMap<String, Integer> getThreeFoldRepetition() {
        return threeFoldRepetition;
    }

    public List<Integer> getTilesOpponentControls() {
        return tilesOpponentControls;
    }
    ...
    public void setPawnDir(int pawnDir) {
    ...
    public void setThreeFoldRepetition(HashMap<String, Integer> threeFoldRepetition) {
        this.threeFoldRepetition = threeFoldRepetition;
    }
    // end: SET methods


    private PieceTracker getPieceTracker(int pieceType, int pieceColor) {
    ...
    public int countMaterial(int color) {
        ...
        material += this.pawns[color].getPieceCount();
        material += this.knights[color].getPieceCount() * 3;
        material += this.bishops[color].getPieceCount() * 3;
        material += this.rooks[color].getPieceCount() * 5;
        material += this.queens[color].getPieceCount() * 9;
    ...
    //
    public boolean playerInCheck() {
        return tilesOpponentControls.contains(kings[colorToMove].getTilesWithPieces()[0]);
    }
